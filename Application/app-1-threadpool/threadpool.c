...
struct threadpool* threadpool_init(int thread_num, int queue_max_num)
{
	...
	while(1) {
		...
		pool->threadlist = (pthread_t*)malloc(sizeof(pthread_t) * thread_num);
		if (NULL == pool->threadlist) {
			printf("failed to malloc pthreadlist\n"); break;
		}
		pool->queue_close = 0;
		pool->pool_close = 0;
		for (int i = 0; i < thread_num; ++i) {
			pthread_create(&(pool->threadlist[i]), NULL, threadpool_function, (void*)pool);
		}
		return pool;
	}
	return NULL;
...
int threadpool_destroy(struct threadpool* pool)
{
	...
	for(int i = 0; i < pool->thread_num; ++i) {
		pthread_join(pool->threadlist[1], NULL);
	}
